...
public class Controller {
    ...
    public ResponseEntity<String>  path2() {
        ...
        doNothingButSleepForSomeTime();
        String response = restTemplate.getForObject("http://localhost:8082/hello/framework", String.class);
        return ResponseEntity.ok("response from /path2 + " + response);
    }
    
    @GetMapping("/framework")
    public ResponseEntity<String>  framework() {
        logger.info("Incoming request at {} at /framework", applicationName);
        doNothingButSleepForSomeTime();
        return ResponseEntity.ok("response from /framework ");
    }
